...
import SliderDeck, { Button } from "./controls/SlideControls";
import Slider from "./controls/ForceSlider";
...
export class ForceLayout extends React.Component {
  ...
  updateForce = force => newState => {
    ...
    this.setState(obj);
    this.simulation.force(force, forces[force](this.state[force].strength));
  };

  updateSize = (e, startSize) => {
    this.setState({ startSize });
    window.localStorage.setItem("startSize", startSize);
  };
      ...
      <div id="holder">
        <SliderDeck>
          ...
          <Slider
            ...
            max={50}
            defaultValue={1}
            update={this.updateForce("center")}
            label="center"
          />
          <Slider
            min={-50}
            max={50}
            value={charge.strength}
            defaultValue={0}
            update={this.updateForce("charge")}
            label="charge"
          />
        </SliderDeck>
      </div>
